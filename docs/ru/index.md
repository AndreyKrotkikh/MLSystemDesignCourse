...
- Требования к валидации:
  - Модель спроса должна описывать/предсказывать метрики на прошедших периодах промо/не промо на выбранных сегментах пользователей в выбранные периоды времени
...
- Может ли быть утечка данных?:
  - В случае создания модели на кривых эластичности - дата лика быть не может (это сильное допущение - нужно подискутировать)
...
- Как модель делает предсказания?:
  ...
  - Режим в абсолютах:
    - Берем существующие кривые эластичности как функции спроса от наценки на базовый тариф
  ...
  - Режим в пропорциях:
    - Берем существующие **нормализованные** кривые эластичности как функции спроса от наценки на базовый тариф
...
- Как будем валидировать модель?:
  - Сопоставляем с историческими данными, мы валидируем как модель описывает поведение, т.е. соответствуют ли кривые эластичности тому, что было на истории, так и расчет интересующих нас метрик, т.е. соответствуют ли аплифты метрик тому, что мы реально наблюдали
